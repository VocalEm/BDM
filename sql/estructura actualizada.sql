...
BEGIN
    CASE p_opcion
        ...
        WHEN 5 THEN
            ...
            WHERE p.ESTATUS = 1 AND u.ESTATUS = 1;
        ...
        WHEN 7 THEN
            ...
            FROM publicaciones p
            INNER JOIN usuarios u ON u.ID_USUARIO = p.ID_USUARIO
            WHERE p.ESTATUS = 1 AND p.ID_USUARIO = p_ID_USUARIO AND u.ESTATUS = 1;

        WHEN 8 THEN
            INSERT INTO comentarios (ID_USUARIO, COMENTARIO, ID_PUBLICACION)
            VALUES (p_ID_USUARIO, p_DESCRIPCION, p_ID_PUBLICACION);
            SELECT TRUE AS correcto;
        ...
        WHEN 9 THEN
            SELECT
               ...
               a.ID_USUARIO,
                ...
                TIPO_IMG,
                USERNAME
            FROM COMENTARIOS_USUARIOS a
            WHERE ID_PUBLICACION = p_ID_PUBLICACION AND ESTATUS = 1;

        WHEN 10 THEN
            INSERT INTO reacciones (ID_USUARIO, ID_PUBLICACION)
            VALUES (p_ID_USUARIO, p_ID_PUBLICACION);
            SELECT TRUE AS correcto;

        WHEN 11 THEN
            SELECT u.ID_USUARIO, ID_PUBLICACION 
            FROM reacciones r
            INNER JOIN usuarios u ON u.ID_USUARIO = r.ID_USUARIO
            WHERE r.ID_USUARIO = p_ID_USUARIO AND r.ID_PUBLICACION = p_ID_PUBLICACION AND u.ESTATUS = 1;
        ...
        WHEN 13 THEN
            ...
            FROM publicaciones p
            INNER JOIN usuarios u ON u.ID_USUARIO = p.ID_USUARIO
            WHERE p.ESTATUS = 1 AND p.CATEGORIA = p_CATEGORIA AND u.ESTATUS = 1;
        ...
        WHEN 14 THEN
            ...
            FROM publicaciones p
            INNER JOIN usuarios u ON u.ID_USUARIO = p.ID_USUARIO
            WHERE p.ESTATUS = 1 AND p.DESCRIPCION LIKE CONCAT('%', p_DESCRIPCION, '%') AND u.ESTATUS = 1;
...
BEGIN
	CASE p_opcion
		...
		-- obtener todos los tableros de un usuario activo
		WHEN 2 THEN
			SELECT
				t.ID_TABLERO,
				t.ID_USUARIO, 
			...
			WHERE t.ID_USUARIO = p_ID_USUARIO AND u.ESTATUS = 1;
		...
		-- obtener publicaciones del tablero (solo de usuarios activos)
		WHEN 4 THEN
			SELECT 
				pt.ID_TABLERO,
				pt.ID_PUBLICACION,
				...
				pt.TIPO_IMG,
				pt.IMAGEN
			FROM PUBLICACIONES_TABLERO pt
			INNER JOIN usuarios u ON u.ID_USUARIO = pt.ID_USUARIO
			WHERE pt.ID_TABLERO = p_ID_TABLERO AND u.ESTATUS = 1;

		-- verifica si una publicacion esta guardada por un usuario activo
		WHEN 5 THEN
			SELECT
				vg.ID_PUBLICACION,
				vg.ID_USUARIO
			FROM VERIFICA_GUARDADO vg
			INNER JOIN usuarios u ON u.ID_USUARIO = vg.ID_USUARIO
			WHERE vg.ID_PUBLICACION = p_ID_PUBLICACION
			  AND vg.ID_USUARIO = p_ID_USUARIO
			  AND u.ESTATUS = 1;

		-- eliminar publicación de un tablero solo si el usuario está activo
		WHEN 6 THEN
			DELETE td
			FROM tablero_detalle td
			JOIN tableros t ON td.ID_TABLERO = t.ID_TABLERO
			INNER JOIN usuarios u ON u.ID_USUARIO = t.ID_USUARIO
			WHERE t.ID_USUARIO = p_ID_USUARIO AND td.ID_PUBLICACION = p_ID_PUBLICACION AND u.ESTATUS = 1;
			SELECT TRUE AS correcto;

		-- obtener info de un tablero específico (si el dueño está activo)
		WHEN 7 THEN
			SELECT 
				t.ID_TABLERO,
				t.ID_USUARIO,
			...
			FROM tableros t
			INNER JOIN usuarios u ON u.ID_USUARIO = t.ID_USUARIO
			WHERE t.ID_TABLERO = p_ID_TABLERO AND u.ESTATUS = 1;
...
BEGIN
    CASE p_opcion
        ...
        WHEN 2 THEN
            UPDATE usuarios SET ESTATUS = 0 WHERE ID_USUARIO = p_ID_USUARIO;
            UPDATE publicaciones SET ESTATUS = 0, CONTADOR_LIKES = 0 WHERE ID_USUARIO = p_ID_USUARIO;
            SELECT TRUE AS correcto;
        ...
        WHEN 4 THEN
            SELECT 
                ...
                FECHA_NACIMINENTO, SEXO, USERNAME, `PASSWORD`, FOTO_PERFIL,
                ESTATUS, PRIVACIDAD, FECHA_REGISTRO, TIPO_IMG,
                SEGUIDORES, SEGUIDOS, PUBLICACIONES
            FROM usuarios
            WHERE ID_USUARIO = p_ID_USUARIO AND ESTATUS = 1;
        ...
        WHEN 8 THEN
            ...
            WHERE ID_USUARIO_SEGUIDO = p_ID_SEGUIDO AND ID_USUARIO_SEGUIDOR = p_ID_USUARIO;
            SELECT TRUE AS correcto;
        ...
        WHEN 12 THEN
            ...
            WHERE ID_USUARIO_SEGUIDO = p_ID_USUARIO AND ID_USUARIO_SEGUIDOR = p_ID_SEGUIDO;
        ...
        WHEN 13 THEN
            ...
            WHERE ID_USUARIO_SEGUIDO = p_ID_USUARIO AND ESTATUS = 1;
        ...
        WHEN 15 THEN
            ...
            WHERE ID_USUARIO_SEGUIDOR = p_ID_USUARIO AND ESTATUS = 1;
...
/*!50001 VIEW `comentarios_usuarios` AS select `a`.`ID_COMENTARIO` AS `ID_COMENTARIO`,`a`.`ID_USUARIO` AS `ID_USUARIO`,`a`.`COMENTARIO` AS `COMENTARIO`,`a`.`ID_PUBLICACION` AS `ID_PUBLICACION`,`b`.`FOTO_PERFIL` AS `FOTO_PERFIL`,`b`.`TIPO_IMG` AS `TIPO_IMG`,`b`.`USERNAME` AS `USERNAME`,`b`.`ESTATUS` AS `ESTATUS` from (`comentarios` `a` join `usuarios` `b` on((`a`.`ID_USUARIO` = `b`.`ID_USUARIO`))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `seguidos_usuarios` AS select `a`.`ID_USUARIO` AS `ID_USUARIO`,`a`.`NOMBRE` AS `NOMBRE`,`a`.`APELLIDO_PATERNO` AS `APELLIDO_PATERNO`,`a`.`APELLIDO_MATERNO` AS `APELLIDO_MATERNO`,`a`.`CORREO` AS `CORREO`,`a`.`FECHA_NACIMINENTO` AS `FECHA_NACIMINENTO`,`a`.`SEXO` AS `SEXO`,`a`.`USERNAME` AS `USERNAME`,`a`.`PASSWORD` AS `PASSWORD`,`a`.`FOTO_PERFIL` AS `FOTO_PERFIL`,`a`.`ESTATUS` AS `ESTATUS`,`a`.`PRIVACIDAD` AS `PRIVACIDAD`,`a`.`FECHA_REGISTRO` AS `FECHA_REGISTRO`,`a`.`TIPO_IMG` AS `TIPO_IMG`,`a`.`SEGUIDORES` AS `SEGUIDORES`,`a`.`SEGUIDOS` AS `SEGUIDOS`,`a`.`PUBLICACIONES` AS `PUBLICACIONES`,`b`.`ID_USUARIO_SEGUIDOR` AS `ID_USUARIO_SEGUIDOR` from (`usuarios` `a` join `seguidores` `b` on((`a`.`ID_USUARIO` = `b`.`ID_USUARIO_SEGUIDO`))) where (`b`.`ESTATUS` = 1) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-05-21 20:21:00
